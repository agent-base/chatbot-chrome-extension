...
async function embedChatbot() {
  const agentbaseChatbotConfig = window.agentbaseChatbotConfig;
  if (!agentbaseChatbotConfig) {
    console.warn('AgentBase Chatbot Url is empty or is not provided');
    return;
  }
  const openIcon = `<svg
            id="openIcon"
  ...
  function createIframe() {
    ...
    iframe.id = 'agentbase-chatbot-bubble-window'
    iframe.src = agentbaseChatbotConfig.chatbotUrl
  ...
  const targetButton = document.getElementById("agentbase-chatbot-bubble-button");

  if (!targetButton) {
    // create button
    const containerDiv = document.createElement("div");
    containerDiv.id = 'agentbase-chatbot-bubble-button';
    ...
    containerDiv.addEventListener('click', function () {
      const targetIframe = document.getElementById('agentbase-chatbot-bubble-window');
      if (!targetIframe) {
        createIframe();
